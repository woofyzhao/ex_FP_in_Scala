...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def mostRetweetAcc(acc: Tweet): Tweet = {
    if (elem.retweets > acc.retweets) right.mostRetweetAcc(left.mostRetweetAcc(elem))
    else right.mostRetweetAcc(left.mostRetweetAcc(acc))
  }
  
  def mostRetweeted: Tweet = mostRetweetAcc(elem)
    
  def descendingByRetweet: TweetList = {
    val rt = mostRetweeted
    new Cons(rt, this.remove(rt).descendingByRetweet)
  }
