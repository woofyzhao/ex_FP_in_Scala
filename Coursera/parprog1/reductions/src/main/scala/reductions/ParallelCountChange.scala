...
object ParallelCountChange {
  ...
  def parCountChange(money: Int, coins: List[Int], threshold: Threshold): Int = {
    if (threshold(money, coins) || money <= 0 || coins.isEmpty) countChange(money, coins)
      ...
      val (l, r) = parallel(parCountChange(money - coins(0), coins, threshold), 
  ...
  def moneyThreshold(startingMoney: Int): Threshold = (m, list) => {
    m <= startingMoney * 2 / 3
  }

  /** Threshold heuristic based on the total number of initial coins. */
  def totalCoinsThreshold(totalCoins: Int): Threshold = (m, coins) => {
